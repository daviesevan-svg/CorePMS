...
- `mix channex.doctor` = end-to-end diagnostic: config, connectivity, link completeness, live ARI readback vs locally computed values, feed health, last push job. Exits non-zero on failure (CI-able). For setting up a fork against a new Channex account, use the `connect-channex` skill (`.claude/skills/connect-channex/`) — it carries the full playbook + gotchas.
- **Outbound flow:** PubSub events → `Listener` (3s debounce, accumulates scopes) → `Workers.PushAri` (queue `:channex`, unique 15s). **Pushes are scoped to what changed:** booking events → availability only (tiny after run-length compression); inventory override edits → `push_restrictions_for/1` delta for just the touched `{room_type, date, field}` cells, sending ONLY the changed Channex keys (a price edit sends `rate` alone — Channex applies partial restriction updates; past dates dropped — Channex rejects them); content/YAML edits → full ARI. Availability comes from non-cancelled stays in Postgres (holds count as occupied), rates from `Hospex.Content.Pricing` (rate-plan YAML: base × seasonal × dow; sent in cents). Daily cron does a full drift-correcting push.
- **Inbound flow:** cron polls `booking_revisions/feed` every minute → `Ingest.apply_revision` → ack. `new` creates a local booking (room auto-assigned within the mapped type, preferring free rooms; falls back to first room and lets the calendar's overbooking lane flag it; `src` mapped Booking.com→BC, Airbnb→AB, Expedia→EX, else ota). `cancelled` cancels via the booking link. `modified` does a 3-way merge against the last-synced OTA state (`base_revision` on `Hospex.Channex.Reservation`, seeded on `new` + every applied `modified`): if the hotel hasn't changed the booking's room shape since (`Reconcile.hotel_touched?/2` false — compares rooms/dates/occupancy), the revision is auto-applied wholesale **including structural changes** (add/update/remove stays — `Bookings.remove_stay/2` drops a room) and the base advances + an `:ota_modified` event logs; if the hotel HAS touched it, the revision is parked on the reservation (`status: "pending"` + a field-level `conflicts` diff) for staff to Accept/Deny, with an `:ota_reconcile` event logged and the revision acked. Unmapped room type → left un-acked to retry; `modified` with no local link → treated as `new`. Failed revisions stay un-acked and retry next poll. Account-wide feed: revisions for properties without a local link are acked + skipped.
- **One price, everywhere:** the inventory page's defaults come from `Hospex.Content.InventoryDefaults` (rate/min-stay via `Pricing` from the primary plan — no more MockInventory), and `push_restrictions` layers the same inventory overrides on top (rate, min_stay, closed→stop_sell, cta/ctd→closed_to_arrival/departure). What staff see on /inventory is exactly what the OTAs get; inventory edits push live via the Listener. Overrides are persisted in Postgres (`Hospex.Inventory.Store` → `inventory_overrides` table, one row per `{room_type_id, date}` with a nullable column per field; NULL = no override), so they survive a restart and the next push won't revert OTA rates.
...
- **Channex: reconciliation Accept/Deny UI.** Modifications to bookings the hotel has touched locally are parked as `pending` `channex_reservations` rows with a field-level `conflicts` diff (and an `:ota_reconcile` event), but there's no staff UI yet to Accept (apply the OTA's values + advance base) or Deny (keep local + advance base) — they only surface in the History tab. Build that on the planned `/settings/channels` page or a booking-drawer panel. Also: switch inbound from per-minute polling to Channex webhooks; a `/settings/channels` page showing sync status + link health; sync all rate plans (not just the primary `CHANNEX_RATE_PLAN`) once the inventory UI grows a plan dimension.
