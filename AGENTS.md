...
- **Inbound flow:** cron polls `booking_revisions/feed` every minute → `Ingest.apply_revision` → ack. `new` creates a local booking (room auto-assigned within the mapped type, preferring free rooms; falls back to first room and lets the calendar's overbooking lane flag it; `src` mapped Booking.com→BC, Airbnb→AB, Expedia→EX, else ota). `cancelled` cancels via the booking link. `modified` reconciles the linked booking: when the room structure is unchanged (same multiset of room types) it applies new dates/occupancy/price/contact via `Bookings.update_multi_stay_booking/3` and logs an `:ota_modified` event; a structural change (room added/removed/retyped) is recorded as an `:ota_reconcile` event on the booking history for a human and acked; an unmapped room type is left un-acked to retry; a `modified` with no local link is treated as `new`. Failed revisions stay un-acked and retry next poll. Account-wide feed: revisions for properties without a local link are acked + skipped.
